...
test("release identities and versions agree across every runtime surface", () => {
  ...
  assert.match(bridge, /version:\s*"2\.0\.0"/);
  assert.match(handlers, /\\"version\\":\\"2\.0\.0\\"/);
});

test("release verifier accepts the exact tag and rejects a mismatched tag", () => {
  const accepted = spawnSync(process.execPath, [releaseVerifier, "--tag", "v2.0.0"], {
    cwd: packageRoot,
    encoding: "utf8",
  });
...
test("current README and concise changelog describe the 2.0 release", () => {
  const readme = read("README.md");
  const changelog = read("CHANGELOG.md");
  const currentRelease = changelog.split("## [1.0.0]")[0];

  assert.match(readme, /## Install version 2\.0\.0/);
  ...
  assert.match(currentRelease, /## \[2\.0\.0\] - 2026-09-04/);
  for (const heading of ["Breaking", "Added", "Changed", "Fixed"]) {
    assert.match(currentRelease, new RegExp(`### ${heading}`));
  }
